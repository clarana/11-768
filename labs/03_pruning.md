...
4. Write functions to calculate the sparsity level (using the percent of buffers that are 0):
    -  for each parameter,
    -  for all pruned parameters, and
    -  for the model overall
   **And report each of these values:** the sparsity level for each parameter, across all pruned parameters, and for the model overall.
